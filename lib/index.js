...
*/
function profileNameOf(ctx, home = homeOf(ctx)) {
	...
	const named = at < 0 ? void 0 : parts[at + 1];
...
*   说成「一个都没有」。
*/
async function sessionTotals(ctx) {
	const query = ctx.get("sessionQuery");
	if (query === void 0 || typeof query.listSessions !== "function") return void 0;
	try {
...
async function collectPresets(ctx) {
	...
	const presets = await Promise.all(list.map(async (p) => {
		...
		const row = {
			...
			...totals === void 0 ? {} : { sessionsTotal: totals.counts.get(p.id) ?? 0 },
			...facts,
			...composition
		};
	...
	return {
		...
		totalsKnown: totals !== void 0,
		...totals === void 0 ? {} : { sessionsUnnamed: totals.unnamed },
		service: "ok"
	};
}
/**
...
async function loadPatch(text) {
	const d = await yamlDialect();
	if (d === null) return { kind: "unavailable" };
	try {
		return {
			kind: "ok",
			value: d.yaml.load(text, { schema: d.schema })
		};
	} catch (error) {
		return {
			kind: "error",
...
* @param text - 补丁文件的全部文本。
*/
async function patchTextProblem(text) {
	const loaded = await loadPatch(text);
	return loaded.kind === "error" ? loaded.problem : void 0;
}
/**
...
*/
async function patchListProblem(text) {
	const loaded = await loadPatch(text);
	if (loaded.kind === "error") return loaded.problem;
	if (loaded.kind === "unavailable") return void 0;
	return Array.isArray(loaded.value) ? void 0 : "补丁层顶层必须是一个列表";
}
/**
* 在补丁文件上执行一次禁用 / 启用。
...
async function applyToggle(opts) {
	...
	if (broken !== void 0) return {
		ok: false,
		reason: "refused",
		message: `补丁文件现在读不回来，先修好它再点：${broken}`
	};
	if (flowStyleList(text)) return {
		ok: false,
		reason: "refused",
	...
	if (action !== "unchanged") {
		...
		if (problem !== void 0) return {
			ok: false,
			reason: "refused",
			message: `这次改写会写出读不回来的补丁文件，已经放弃：${problem}`
		};
		try {
...
*     结果是「服务被杀掉且再也起不来」（#229）。
*
* 安全边界：这个模块只被 RPC 通道调用，而 `connection.rpc.handle` 注册出来的通道
* 一律是已认证的——宿主自己挡掉未认证、非本机与跨站点的请求，这里不再重复做同一件事。
* （dsh 0.1.2 起逐通道的 `authority` 策略被宿主级的信任围栏取代，那个参数已经没了。）
*/
...
*/
function embeddedHost(versions = process.versions, execPath = process.execPath) {
...
function layersOf(ctx) {
	...
	return rebuildLayers({
		profileName: profileNameOf(ctx, home),
		anchor: ownAnchor(),
		home
	});
}
...
function apply(ctx) {
	...
	ctx.effect(() => ctx.connection.register(ctx, INSIGHT_CHANNEL, createInsightHandler(ctx)), "dsh-insight: rpc channel");
}
//#endregion
