...
- `$DSH_HOME/settings.yaml` — legacy settings file from older dsh; 0.1.7 imports it once on first start and renames it to `settings.yaml.imported`, so it is listed only when present
